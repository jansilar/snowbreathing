...
model ConeCompGrad
  ...
  Modelica.Blocks.Sources.CombiTimeTable table(columns = 2:4, extrapolation = Modelica.Blocks.Types.Extrapolation.HoldLastPoint, fileName = fileGrad, smoothness = Modelica.Blocks.Types.Smoothness.ConstantSegments, tableName = "GRAD", tableOnFile = true)  annotation(
...
equation
  r = table.y[1];
algorithm
  when r > rOld then //r has changed, time of new grad measurement reached
    rOld := r;
    rGT[pre(i)] := table.y[1];
    CO2GT[pre(i)] := table.y[2];
    O2GT[pre(i)] := table.y[3];
    ix := 1;
    Modelica.Utilities.Streams.print("t = " + String(time) + "\n r = " + String(r) + "\n");
    while omega.x[ix] < r + R_in loop
      ix := ix + 1;
    ...
    CO2GM[ix] := CO2[ix-1];// + (CO2[ix] - CO2[ix-1])/omega.dx *(r - omega.x[ix-1]);
    i := pre(i) + 1;
  end when;
