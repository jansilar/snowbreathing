  ...
  equation
    ...
    connect(gain.u, concentrationMeasure1.concentration) annotation (Line(
          points={{-79,-17.4},{-83.5,-17.4},{-83.5,-22},{-78,-22}}, color={0,0,
            127}));
    connect(gain.y, outerFracFlow.fractionIn) annotation (Line(points={{-79,-1.3},
            {-79.5,-1.3},{-79.5,0},{-70,0}}, color={0,0,127}));
  ...
  package OriginalDoLoi
    ...
    equation
      ...
      connect(atmos_pressure.y, unlimitedVolume.pressure) annotation (Line(
            points={{-89.878,-86.6202},{-89.878,-84.3101},{-90,-84.3101},{-90,-80}},
            color={0,0,127}));
      connect(unlimitedVolume.y, R_ml.q_in) annotation (Line(
          points={{-90,-60},{-90,60},{-86,60}},
          color={0,0,0},
...
<p><span style=\" font-family:'Courier New'; font-size:12pt; color:#008b00;\">Compute partial pressure of gas from its fraction in medium, P_atm and P_ws. </span></p></body></html>"));
  ...
  model CirculationGas
    ...
    Physiolibrary.Types.BusConnector lungMech_bus annotation (Placement(
        ...
        iconTransformation(
          ...
          extent={{-20,-20},{20,20}},
          rotation=0)));
    Modelica.Blocks.Math.Gain frac_alveolar(k=m3PerMole) annotation (Placement(
          transformation(
          extent={{-7,-7},{7,7}},
          rotation=180,
          origin={89,35})));
    ...
    Modelica.Blocks.Math.Gain frac_deadSpace(k=m3PerMole) annotation (Placement(
          transformation(
          extent={{-4,-4},{4,4}},
          rotation=180,
          origin={-22,52})));
    Physiolibrary.Chemical.Components.Substance subs_tube(useNormalizedVolume=
          false, solute_start(displayUnit="mmol") = 1e-08)
    ...
    Physiolibrary.Types.Constants.VolumeConst vol_tube(k(displayUnit="ml") =
        0.001) annotation (Placement(transformation(
          ...
          rotation=180,
          origin={-68,88})));
    Physiolibrary.Chemical.Components.Stream tracheaStream1(
        useSolutionFlowInput=true)
  ...
  equation
    ...
    connect(tracheaStream.q_out, subs_dead.q_out) annotation (Line(
        ...
        thickness=1));
    connect(tracheaStream.solutionFlow, lungMech_bus.totalFlow) annotation (
        Line(points={{-40,77},{-40,104},{-21,104},{-21,105}}, color={0,0,127}),
        Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}}));
    connect(subs_dead.solutionVolume, lungMech_bus.volumeDeadspace) annotation
      (Line(points={{-14,74},{-14,104},{-21,105}}, color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}}));
    connect(subs_dead.q_out, bronchiolyStream.q_in) annotation (Line(
    ...
    connect(conc_deadSpace.q_in, subs_dead.q_out) annotation (Line(
        ...
        color={107,45,134},
        thickness=1));
    connect(subs_tube.q_out, tracheaStream.q_in) annotation (Line(
        points={{-70,70},{-50,70}},
        color={107,45,134},
        thickness=1));
    connect(tracheaStream1.solutionFlow, lungMech_bus.totalFlow) annotation (
        Line(points={{-100,77},{-100,104},{-21,104},{-21,105}}, color={0,0,127}),
        Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}}));
    connect(tracheaStream1.q_out, subs_tube.q_out) annotation (Line(
        points={{-90,70},{-70,70}},
        color={107,45,134},
        thickness=1));
    connect(subs_tube.solutionVolume, vol_tube.y)
      annotation (Line(points={{-74,74},{-74,88},{-73,88}}, color={0,0,127}));
    connect(conc_Tube.concentration, frac_tube.u) annotation (Line(points={{-70,
            52.8},{-77.2,52.8},{-77.2,52}}, color={0,0,127}));
    connect(conc_Tube.q_in, subs_tube.q_out) annotation (Line(
        points={{-70,56},{-70,70}},
        color={107,45,134},
        thickness=1));
    connect(tracheaStream1.q_in, port_a) annotation (Line(
        points={{-110,70},{-120,70},{-120,0},{-130,0}},
        color={107,45,134},
        thickness=1));
    annotation (Icon(coordinateSystem(preserveAspectRatio=false, extent={{-140,
              -120},{120,120}})), Diagram(coordinateSystem(preserveAspectRatio=
  ...
  model TestCircGas
    CirculationGas circulationGas(vol_tube(k=1e-05))
  ...
  equation
    ...
    connect(vol_sac.y, substance.solutionVolume)
      annotation (Line(points={{17,82},{-14,82},{-64,54}}, color={0,0,127}));
  ...
  model Advection
    ...
    constant Integer N=100 "number of the discrete elements of the tube";
    parameter Physiolibrary.Types.Position length=10 "tube length";
    ...
    //   parameter Physiolibrary.Types.VolumeFlowRate Q "medium flow";
    Physiolibrary.Types.Velocity v=q/area_tube_crosssection "velocity of the ";
  ...
  equation
    ...
    for i in 2:N - 1 loop
      der(c[i]) = -v*(c[i + 1] - c[i - 1])/(2*length/N);
    end for;

    if v > 0 then
      //positive flow from 1 to end
      q_in.q = q_in.conc*q;
      q_out.q = -c[end]*q;
  ...
  model TestAdvection
    ...
    Advection advection(SolutionFlow=8.3333333333333e-05, d=0.01)
      annotation (Placement(transformation(extent={{-20,0},{0,20}})));
  equation
